...
SET budget_normalized = ROUND(budget_actual / (slot_mod * drop_mod * misc_mod), 2)
...
/*======================================================================================================================================
  Slot Modifier Sanity Check
  --------------------------------------------------------------------
  Purpose:
    Verify harmonization of Equipment slot modifiers after applying the +25 % scale.
    Confirms that normalized budgets for Equipment now align with Weapons (~1.00 ± 0.05).
======================================================================================================================================*/

-- Quick summary of the applied Equipment slot_mod values
SELECT
    InventoryType,
    InventoryName,
    multiplier AS new_slot_mod
FROM ACSBV3_doc_mod_slot_equipment
ORDER BY InventoryType;

-- Cross-family comparison of mean normalization ratios (sanity check)
SELECT
    family,
...
-- Equipment slot-level consistency check
SELECT
    InventoryType,
    ROUND(AVG(budget_normalized / budget_actual),3) AS mean_norm_ratio,
...
ORDER BY mean_norm_ratio DESC;

-- Expected pattern:
--   • family mean_norm_ratio values ˜ 1.00 ± 0.05 across Equipment and Weapons
--   • Slot-level means roughly centered near 1.00 with small std_dev (<0.2)
--   • Any large deviation indicates further slot-specific fine-tuning may be needed
/*======================================================================================================================================
  End of Slot Modifier Sanity Check
======================================================================================================================================*/
